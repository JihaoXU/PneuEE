...
class URRobot(object):
    ...
    ##===========================================
    def get_myPose(self, wait=True):
        """
        return measured pose in TCP
        if wait==True, waits for next packet before returning
        """
        return self.rtmon.getMyPose(wait)

    def get_mySpeed(self, wait=True):
        """
        return measured speed in TCP
        if wait==True, waits for next packet before returning
        """
        return self.rtmon.getMySpeed(wait)
