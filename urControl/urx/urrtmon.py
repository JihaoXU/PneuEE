...
class URRTMonitor(threading.Thread):
    ...
    def __init__(self, urHost):
        ...
        #The TCP pose and speed in Cartesian coordinates
        self._myPose = None
        self._mySpeed = None
    ...
    ##=============================================
    def getMyPose(self, wait=False, timestamp=False):
        """ Get the tool pose. The returned tool pose is a
        six-vector of three positions and three rotations."""
        if wait:
            self.wait()
        with self._dataAccess:
            tcp_pose = self._myPose
            if timestamp:
                return self._timestamp, tcp_pose
            else:
                return tcp_pose

    def getMySpeed(self, wait=False, timestamp=False):
        """ Get the tool speed. The returned tool speed is a
        six-vector of three linear velocity and three angular velocity."""
        if wait:
            self.wait()
        with self._dataAccess:
            tcp_speed = self._mySpeed
            if timestamp:
    ...
    def __recv_rt_data(self):
        ...
        with self._dataAccess:
            ...
            #The TCP pose and speed in Cartesian coordinates
            self._myPose = np.array(unp[55:61])
            self._mySpeed = np.array(unp[61:67])
